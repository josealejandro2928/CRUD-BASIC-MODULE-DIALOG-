...
function run(schema){
  let result = '';
  for(let key in schema){
    if((schema[key].type=='LONG-STRING' || (schema[key].type=='JSON' && schema[key].longTxt)) 
    && schema[key].useCkEditor){
      result+=`import * as Editor from 'src/assets/js/ckeditor/build/ckeditor';
